...
ggplot()+
  geom_point(data = cost_gas_data, aes(x= q_low_gallons, y = price_dollars),color="cyan3", alpha = .2)+
  stat_function(aes(x=0, color = "Low Consumer"), fun = low_demand)+ 
  geom_point(data = cost_gas_data, aes(x= q_high_gallons, y = price_dollars),color="chartreuse3", alpha = .2)+
  stat_function(aes(x=0, color = "High Consumer"), fun = high_demand)+
  geom_point(data = cost_gas_data, aes(x= q_aggregrate, y = price_dollars),color="coral1", alpha = .2)+
  stat_function(aes(x=0, color = "Aggregate"), fun = agg_demand, size = .8)+
  stat_function(aes(x=0, color = "Supply"), fun = supply, size = .6)  +
  xlim(0,800000)+ 
  scale_y_continuous(breaks=c(0,3,6,9,12,15,18),limits = c(0,18)) +
  labs(title = "Gas Supply and Demand for Various Consumers", x = "Gallons", y = "Price USD", colour = "Curve") +
...
net_consumer_benefit <- gross_consumer_benefit - (p_agg*agg_curve_intersect)
```    
...
high_producer_benefit <- integrate(supply,lower=0,upper=high_curve_intersect)$value
low_producer_benefit <- integrate(supply,lower=0,upper=low_curve_intersect)$value
...
high_net_consumer_benefit <- high_gross_consumer_benefit - (p_agg*q_high_3)
low_net_consumer_benefit <- low_gross_consumer_benefit - (p_agg*q_low_3)
```
...
high_producer_benefit_tax <- integrate(supply,lower=0,upper=high_tax_curve_intersect)$value
low_producer_benefit_tax <- integrate(supply,lower=0,upper=low_tax_curve_intersect)$value
...
# get P* and q* for each curve 
p_1_tax<- 4
q_high_4<- inverse_high_demand(p_1_tax)
q_low_4<- inverse_low_demand(p_1_tax)

# Net Consumer Benefit
high_net_consumer_welfare_tax <- high_gross_consumer_welfare_tax - (p_1_tax*q_high_4)
low_net_consumer_welfare_tax <- low_gross_consumer_welfare_tax - (p_1_tax*q_low_4)
```
...
low_tax_revenue <- t*low_tax_curve_intersect_var
  ...
  high_producer_welfare_var <- integrate(supply,lower=0,upper=high_tax_curve_intersect_var)$value  
  low_producer_welfare_var <- integrate(supply,lower=0,upper=low_tax_curve_intersect_var)$value  
  
  high_welfare_var <- integrate(high_demand,lower=0,upper=high_tax_curve_intersect_var)$value + redis_high - high_tax_revenue 
  low_welfare_var <- integrate(low_demand,lower=0,upper=low_tax_curve_intersect_var)$value +redis_low -local_env_cost_tax_var-low_tax_revenue

  
  return(data.frame(tax=t,
                    high_welfare = high_welfare_var,
                    low_welfare = low_welfare_var,
                    producer_welfare =  low_producer_welfare_var+high_producer_welfare_var,
                    low_producer_welfare= low_producer_welfare_var,
                    high_producer_welfare= high_producer_welfare_var))
...
# Visualize
ggplot()+
  stat_function(aes(x=0, color = "Low"), fun = low_demand_h)+
  stat_function(aes(x=0, color = "High"), fun = high_demand_h)+
  stat_function(aes(x=0, color = "Supply"), fun = supply, size = .6)+ 
  stat_function(aes(x=0, color = "Aggregate"), fun = agg_demand_h, size = .8)+ 
xlim(0,400000)+ 
  scale_y_continuous(breaks=c(0,3,6,9,12,15,18),limits = c(0,10)) +
  labs(title = "Gas Supply and Adjusted Demand for Various Consumers", x = "Gallons", y = "Price USD", colour = "Curve") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_bw() +
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 15, b = 0, l = 15))) +
  theme(axis.title.x = element_text(margin = margin(t = 15, r = 0, b = 15, l = 0))) 
...
xlim(0,400000)+ 
  scale_y_continuous(breaks=c(0,3,6,9,12,15,18),limits = c(0,10)) +
  labs(title = "?", x = "Gallons", y = "Price USD", colour = "Curve") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_bw() +
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 15, b = 0, l = 15))) +
  theme(axis.title.x = element_text(margin = margin(t = 15, r = 0, b = 15, l = 0))) 
