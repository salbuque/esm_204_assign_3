...
supply_tax <- function(q)  4 + 0.0000*q
...
```{r}
# Producer welfare
producer_tax_welfare <- agg_tax_curve_intersect*3
```
...
Total Environmental Cost with tax  $=\$`r comma(total_env_cost_tax)`$
...
$P=`r round(lm_agg$coefficients[1]/2)``r round(lm_agg$coefficients[2]/2,digits=6)`Q\text{ when } Q\geq`r comma(intersect_point_h)`$
